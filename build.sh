...
build_ios_dylib() {
  ...
  run "Combining builds to universal fat library ..." \
    lipo -create -output "${RELEASE_DIR}/LeanplumLocation" \
    "${CURRENTCONFIG_ARMV7_DEVICE_DIR}/Leanplum-iOS-Location/LeanplumLocation.framework/LeanplumLocation" \
    "${CURRENTCONFIG_ARMV7S_DEVICE_DIR}/Leanplum-iOS-Location/LeanplumLocation.framework/LeanplumLocation" \
    "${CURRENTCONFIG_ARM64_DEVICE_DIR}/Leanplum-iOS-Location/LeanplumLocation.framework/LeanplumLocation" \
    "${CURRENTCONFIG_X86_DEVICE_DIR}/Leanplum-iOS-Location/LeanplumLocation.framework/LeanplumLocation" \
    "${CURRENTCONFIG_X8664_SIMULATOR_DIR}/Leanplum-iOS-Location/LeanplumLocation.framework/LeanplumLocation"

  # Copy generated framework
  cp -r "${BUILD_DIR}$ARMV7_DIR/$CONFIGURATION-iphoneos/Leanplum-iOS-Location/LeanplumLocation.framework/" \
    "${RELEASE_DIR}/LeanplumLocation.framework"
  rm -f "${RELEASE_DIR}/LeanplumLocation.framework/Leanplum"
  mv "${RELEASE_DIR}/LeanplumLocation" "${RELEASE_DIR}/LeanplumLocation.framework/"

  # Delete unnecessary headers
  rm -rf "${RELEASE_DIR}/LeanplumLocation.framework/_CodeSignature"
...
zip_ios_dylib() {
  cd "${RELEASE_DIR}"
  zip -r LeanplumLocation.framework.zip LeanplumLocation.framework
  mv LeanplumLocation.framework.zip "$SDK_DIR"
}
